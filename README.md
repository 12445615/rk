...
## 当前存档说明：OSD 标签、数字清晰度与推流流畅度优化

本次存档已经完成视频画面 OSD 显示优化，重点解决检测标签、置信度数字和字体在 RTMP/VLC 推流中发虚、花边、彩色噪点和不清晰的问题。

已完成内容：

- 检测框按类别使用不同颜色，便于区分 `helmet`、`no-helmet`、`no-vest`、`person`、`vest`、`fire`
- 标签和置信度数字改成黑底白字显示
- 标签左侧保留类别色条，颜色与检测框一致
- 置信度数字由整串 `00%~99%` 字符贴图生成，避免单字符拼接导致的抠图感
- 字体显示不再走 RGA 的 RGBA alpha blend 贴图路径
- 黑色底条直接写 NV12 的 Y/UV 平面，白色文字直接写 NV12 的 Y 亮度平面
- 避免小字经过 NV12/YUV420 和 H264 压缩后出现彩色花边
- `vest` 和数字显示清晰度已经完成现场验证

本次也同步优化了推流低延时和流畅度参数：

- 默认推流帧率从 `15fps` 调整为 `30fps`，与摄像头采集帧率对齐
- H264 GOP 调整为 `15` 帧，约 `0.5s` 一个关键帧
- 编码码率从 `2Mbps` 调整为 `3Mbps`
- 码率缓冲从 `300KB` 调整为 `200KB`
- `qmax` 从 `45` 调整为 `38`，减少复杂画面和运动画面的质量波动
...
fps [30:1:fix]
gop 15
bps [2812500:3000000:3187500]
[Child] stream duplicate_frames=1 target_fps=30
[Child][Encode] fps=30.4 avg_ms=2.47 max_ms=25
[Parent][Video] capture_fps=30.2 stream_fps=30.2
```

核心结论：

```text
推流 OSD 小字不要走 RGBA alpha blend 到 NV12，直接写 Y 亮度层最稳。
